...
void MainWindow::onLogin(LoginWindow::LoginData loginData)
{
	...
	if (ret != Ret_Success)
	{
		QMessageBox::critical(pLoginWindow, QString::fromStdWString(L"登录失败"), QString("Login Failed, RetCode = ") + QString::number((int)ret));
		return;
	}
    UnsubscribeAll();
...
void MainWindow::needSubscribe(MsgType msgType, QString codeStr)
{
    ...
    case Msg_SSEL2_Static:
        headerList <<QString::fromWCharArray(L"索引") << "QDSTime" << "Symbol" << "Reserved" << "Time"
    ...
    case Msg_SSEL2_Transaction:
        headerList << QString::fromWCharArray(L"索引") << "QDSTime" << "Symbol" << "Reserved" << "TradeTime"
	...
	case Msg_SSEIOL1_Static:
		headerList << QString::fromWCharArray(L"索引") << "QDSTime" << "Symbol" << "Reserved" << "ContractID"
    ...
    case Msg_SZSEL2_Static:
		headerList << QString::fromWCharArray(L"索引") << "QDSTime" << "Symbol" << "SecurityName" << "SymbolSource"
    ...
    case Msg_SZSEL2_Quotation:
        headerList << QString::fromWCharArray(L"索引") << "QDSTime" << "Symbol" << "Time"<< "SymbolSource"
    ...
    case Msg_SZSEL2_Transaction:
        headerList << QString::fromWCharArray(L"索引") << "QDSTime" << "Symbol" << "SetID" << "RecID"
    ...
    case Msg_SZSEL2_Status:
        headerList << QString::fromWCharArray(L"索引") << "QDSTime" << "Symbol" << "SymbolSource" << "Time"
